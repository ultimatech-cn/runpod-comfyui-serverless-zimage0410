...
if [[ $# -eq 0 ]]; then
  echo "Usage: comfy-node-install <node1> [<node2> 鈥" >&2
  exit 64  # EX_USAGE
fi

log=$(mktemp)

# run installation 鈥?some modes return non-zero even on success, so we
...
if [[ -n "$failed_nodes" ]]; then
  ...
  echo "$failed_nodes" | while read -r n; do echo "  鈥?$n" >&2 ; done
  echo >&2
...
if [[ $cli_status -ne 0 ]]; then
  echo "Warning: comfy node install exited with status $cli_status but no errors were detected in the log 鈥?assuming success." >&2
fi
